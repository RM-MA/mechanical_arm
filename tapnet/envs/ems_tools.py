...
def compute_packing_pos(box, ems_id, corner_id, empty_max_spaces, id_map=None):
    ...
    container_id = np.array(empty_max_spaces[ems_id][2])[0] # get container_id
...
def compute_empty_space(container_size, corners, x_borders, y_borders, id_map, heightmap, empty_space_list, x_side='left-right', y_side='left-right', space_type=0, min_ems_width=0, container_id=0):
    ...
    for corner in corners:
        ...
        for axes in itertools.permutations(range(2), 2):
            ...
            if check_valid_height_layer(h_layer[ x_small:x_large, y_small:y_large], space_type):
...
def compute_ems(id_maps, heightmaps, container_size, min_ems_width=0, ems_type='ems-id-stair', valid_container_mask=None):
    ...
    for container_id in range(len(heightmaps)):
        ...
        # NOTE ems
        # compute_empty_space(container_size, id_left_bottom_corners, x_borders, y_borders, id_map, heightmap, empty_max_spaces, 'left-right', 'left-right', min_ems_width=min_ems_width, container_id=container_id)
        if 'ems' in ems_type:
            if corners is None:
...
def compute_box_ems_mask(box_states, ems, box_num=None, all_container_space=None, heightmaps=None, packing_mask=None, remove_list=None, check_box_stable=True, gripper_size=None, check_z=True, same_height_threshold=0):
    # can place or not
    ...
    if gripper_size is not None:
        ...
        for pair in valid_pair:
            ...
            if ems.shape[1] == 6:
                ...
                container_id = int(ems[ems_idx][6])
            ...
            if max_h - (lz + bz) > gz:
                ems_to_box_mask[ems_idx, box_idx] = False
    ...
    if packing_mask is not None or check_box_stable:
        ...
        for pair in valid_pair:
            ...
            if ems.shape[1] == 6:
                ...
                container_id = int(ems[ems_idx][6])
            lx, ly, lz = ems[ems_idx][:3]
            bx, by, bz = box_states[box_idx]
            ...
            if False:
            # if gripper_size is not None:
                # hw, hl = heightmap.shape

                gw, gl, gh = gripper_size
                ...
                max_h = np.max(hm[ max(0, lx-gw): lx+gw, max(0, ly-gl): ly+gl ])

                if max_h - lz > gh:
                    ems_to_box_mask[ems_idx, box_idx] = False


                # gripper_rigion = heightmap[ gripper_lx: gripper_lx+gripper_w, gripper_ly: gripper_ly+gripper_l ].copy()


                if False:
                # if bx < gx or by < gy:
                    gripper_lz = lz + bz
                    # gripper_lx, gripper_ly

                    gripper_mode = [1,2,3,4,5]
                    # center, left-front, left-back, right-front, right-back

                    for mode in gripper_mode:
                        # left_bottom type
                        if mode == 1:
                            gripper_lx = int(np.floor(lx + bx/2.0 - gx/2.0))
                            gripper_ly = int(np.floor(ly + by/2.0 - gy/2.0))
                        elif mode == 2:
                            gripper_lx = lx
                            gripper_ly = ly
                        elif mode == 3:
                        ...
                        gripper_w, gripper_l = gx, gy
                        if gripper_lx < 0:
                            gripper_w = gx + gripper_lx
                            gripper_lx = 0
                        if gripper_ly < 0:
                            gripper_l = gy + gripper_ly
                            gripper_ly = 0
                        

                        gripper_rigion = heightmap[ gripper_lx: gripper_lx+gripper_w, gripper_ly: gripper_ly+gripper_l ].copy()

                        new_lx = lx - gripper_lx
                        new_ly = ly - gripper_ly

                        gripper_rigion[ new_lx: new_lx+bx, new_ly: new_ly+by ] = 0

                        if (gripper_rigion > gripper_lz).any():
                            ems_to_box_mask[pair[0], pair[1]] = False
                        else:
                            ems_to_box_mask[pair[0], pair[1]] = True
                            ems_box_grasp_mask[pair[0], pair[1]] = mode
                            break
