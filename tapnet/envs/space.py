...
class Space(object):
    ...
    def drop_box(self, box_size, idx, rot_flag, density, setting, allow_unsta=False, same_height_threshold=0):
        if not rot_flag:
            x, y, z = box_size
        else:
            y, x, z = box_size
        ...
        if area_sta and setting != 2:
            ...
            for tmp in self.boxes:
                ...
                if abs(tmp.lz + tmp.z - max_h) < 1e-4 + same_height_threshold:
            ...
            if len(combine_contact_points) > 0:
                ...
                if area < ((1 - self.stable_scale_factor) * (x * y)) * self.area_sta_factor:
                    # print('>>> here a', area, x*y, area / ((1 - self.stable_scale_factor) * (x * y)) )
                    area_sta = False
    ...
    def drop_box_virtual(self, box_size, idx, flag, density, setting,  returnH = False, returnMap = False, same_height_threshold=0):
        if not flag:
            x, y, z = box_size
        else:
            y, x, z = box_size
